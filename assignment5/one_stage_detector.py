...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level.keys():
            ...
            x_cls = torch.swapaxes(x_cls, 1, 2)
            class_logits[key] = x_cls

            # Pass feature map though stem and box regression layer
            x_box = self.stem_box(feats_per_fpn_level[key])
            x_box = self.pred_box(x_box)
            x_box = torch.flatten(x_box, start_dim=2)
            x_box = torch.swapaxes(x_box, 1, 2)
            boxreg_deltas[key] = x_box

            # Pass feature map through stem and center regression layer
            x_ctr = self.stem_box(feats_per_fpn_level[key])
            x_ctr = self.pred_ctr(x_ctr)
            x_ctr = torch.flatten(x_ctr, start_dim=2)
            x_ctr = torch.swapaxes(x_ctr, 1, 2)
            centerness_logits[key] = x_ctr
...
def fcos_get_deltas_from_locations(
    ...
    # left delta: (xc - x1) / stride
    l = ((locations[:, 0] - gt_boxes[:, 0]) / stride).reshape(-1, 1)
    
    # right delta: (x2 - xc) / stride
    r = ((gt_boxes[:, 2] - locations[:, 0]) / stride).reshape(-1, 1)
    
    # top delta: (yc - y1) / stride
    t = ((locations[:, 1] - gt_boxes[:, 1]) / stride).reshape(-1, 1)
    
    # bottom delta: (y2 - yc) / stride
    b = ((gt_boxes[:, 3] - locations[:, 1]) / stride).reshape(-1, 1)
    
    # Merge deltas to one tensor in the LTRB format
    deltas = torch.concat([l,t,r,b], dim=1)
...
def fcos_apply_deltas_to_locations(
    ...
    # Compute box coordinates from deltas, locations and stride
    x1 = (locations[:, 0] - deltas[:, 0] * stride).reshape(-1, 1)
    x2 = (locations[:, 0] + deltas[:, 2] * stride).reshape(-1, 1)
    y1 = (locations[:, 1] - deltas[:, 1] * stride).reshape(-1, 1)
    y2 = (locations[:, 1] + deltas[:, 3] * stride).reshape(-1, 1)
    
    # Merge box coordinates to one tensor in the XYXY format
    output_boxes = torch.concat([x1,y1,x2,y2], dim=1)
    
    # Handle background deltas
    mask = deltas[:, 0] == -1
    output_boxes[mask, 0] = locations[mask, 0]
    output_boxes[mask, 1] = locations[mask, 1]
    output_boxes[mask, 2] = locations[mask, 0]
    output_boxes[mask, 3] = locations[mask, 1]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    nominator = torch.min(deltas[:, 0], deltas[:, 2]) * torch.min(deltas[:, 1], deltas[:, 3])
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        # Replace "pass" statement with your code
        self.backbone = DetectorBackboneWithFPN(out_channels=64)
    ...
    def forward(
        ...
        # Run feature pyramid features through prediction network
